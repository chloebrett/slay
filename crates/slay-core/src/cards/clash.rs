...
pub fn apply(state: &mut CombatState, events: &mut Vec<Event>, grade: Grade, target: usize) {
    let base = match grade { Grade::Base => 14, Grade::Plus => 18 };
...
pub(super) fn def(grade: Grade) -> CardDef {
    ...
    CardDef { name, description: CardDescription::WithDamage { template: "Can only be played if every card in your hand is an Attack. Deal {damage} damage.", base }, energy_cost: Energy(0), card_type: CardType::Attack }
}

pub(super) fn id(grade: Grade) -> &'static str {
    match grade { Grade::Base => "clash", Grade::Plus => "clash-plus" }
}
