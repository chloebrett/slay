...
pub const DEF: EnemyDef = EnemyDef { name: "Spike Slime (L)", max_hp: Hp(67) };
...
pub fn next_move(history: &[Move], rng: &mut impl Rng) -> Move {
    ...
    let mut candidates: Vec<Move> = [
        Move::LargeSpikeFlameTackle,
        Move::LargeSpikeLick, Move::LargeSpikeLick,
    ]
    .into_iter()
...
pub fn on_player_attack_damage(current_hp: Hp, max_hp: Hp) -> Option<EnemyDamageReaction> {
    if current_hp.0 <= max_hp.0 / 2 {
        Some(EnemyDamageReaction {
            ...
            silent_adds: vec![],
            silent_sets: vec![],
            force_move: Some(Move::LargeSpikeSplit),
        })
    } else {
        None
    }
